            ...
            [=]()
    {
        ...
        if(ok1&&ok2)
        {
            ...
            if(flag==1)
            {
                ...
                if(sp==1)
                {
                    temp.sprintf("该猪是黑猪,成长天数为%d天,体重为%0.1fkg",gd,w);
                }
                if(sp==2)
                {
                    temp.sprintf("该猪是小白猪,成长天数为%d天,体重为%0.1fkg",gd,w);
                }
                if(sp==3)
                {
                    temp.sprintf("该猪是大花白猪,成长天数为%d天,体重为%0.1fkg",gd,w);
                }
                ui->textout->setText(temp);
            }
            else
            ...
            [=]()
    {
        ...
        else
        {
            ...
            temp.sprintf("卖出%d只黑猪、%d只小花猪、%d只大花白猪\n"
                         "总共卖出%d只猪,得到%0.1f元",qianpig1-houpig1,qianpig2-houpig2,qianpig3-houpig3,sellpig1-sellpig2,sumprice);
            ui->textout->setText(temp);
            sumday=0;
        }
    }
